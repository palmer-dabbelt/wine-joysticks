...
static void DumpState(DIJOYSTATE * st, int num_buttons)
{
    ...
    printf("----------------------------------\n");
    printf
        ("Ax: (% 5d,% 5d,% 5d)\tRAx: (% 5d,% 5d,% 5d)\nSlider: (% 5d,% 5d)\tPov: (% 5d,% 5d,% 5d,% 5d)\n\n",
         st->lX, st->lY, st->lZ, st->lRx, st->lRy, st->lRz, st->rglSlider[0],
         st->rglSlider[1], st->rgdwPOV[0], st->rgdwPOV[1], st->rgdwPOV[2],
         st->rgdwPOV[3]
        );
...
void DumpDeviceObject(const DIDEVICEOBJECTINSTANCEA * instance)
{
    ...
    static REFGUID guidTypes[] =
        { &GUID_Button, &GUID_XAxis, &GUID_YAxis, &GUID_ZAxis, &GUID_RxAxis,
&GUID_RyAxis, &GUID_RzAxis, &GUID_Slider };
    static const char *guidStrings[] =
        { "Button", "X", "Y", "Z", "Rx", "Ry", "Rz", "Slider" };
...
/*
    ...
    object properties.
*/
static BOOL CALLBACK EnumObjectsCallback(const DIDEVICEOBJECTINSTANCEA *
                                         instance, void *context)
{
    ...
    if (instance->dwType & DIDFT_AXIS) {
        ...
        IDirectInputDevice_SetProperty(joystick->device, DIPROP_RANGE,
                                       &propRange.diph);
    }

    if (instance->dwType & DIDFT_BUTTON)
        joystick->num_buttons += 1;
...
*/
static BOOL CALLBACK EnumCallback(const DIDEVICEINSTANCEA * instance,
                                  void *context)
{
    struct JoystickData *data = context;
    ...
    IDirectInput8_CreateDevice(data->di, &instance->guidInstance,
